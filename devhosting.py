...
def ask_to_upload_file(call):
    ...
    if bot_locked:
        bot.send_message(call.message.chat.id, "⚠️ The bot is currently locked. Please contact the developer @Elite_shot10000.")
        return
    if free_mode or (user_id in user_subscriptions and user_subscriptions[user_id]['expiry'] > datetime.now()):
        ...
        bot.send_message(call.message.chat.id, "⚠️ You need a subscription to use this feature. Please contact the developer @Elite_shot10000")
...
def handle_file(message):
    user_id = message.from_user.id
    if bot_locked:
        bot.reply_to(message, "⚠️ The bot is currently locked. Please contact the developer @Elite_shot10000")
        return
    if free_mode or (user_id in user_subscriptions and user_subscriptions[user_id]['expiry'] > datetime.now()):
        try:
        ...
        bot.reply_to(message, "⚠️ You need a subscription to use this feature. Please contact the developer @Elite_shot10000")
